...
const wordChosen = document.querySelector('.emphasized-word');

for (let i = 0; i < colorBtns.length; i++) {
    if (colorBtns[i].textContent === 'Blue') {
        colorBtns[i].addEventListener('click', function () {
            wordChosen.style.color = 'blue';
        });
    } else if (colorBtns[i].textContent === 'Red') {
        colorBtns[i].addEventListener('click', function () {
            wordChosen.style.color = 'red';
        });
    } else if (colorBtns[i].textContent === 'Green') {
        colorBtns[i].addEventListener('click', function () {
            wordChosen.style.color = 'green';
        });
    } else if (colorBtns[i].textContent === 'Yellow') {
        colorBtns[i].addEventListener('click', function () {
            wordChosen.style.color = 'yellow';
        });
    } else if (colorBtns[i].textContent === 'Black') {
        colorBtns[i].addEventListener('click', function () {
            wordChosen.style.color = 'black';
        });
    };
};